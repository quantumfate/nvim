--- Scaffold generation: the file table, the plan/apply pipeline, and the
--- doctor/sanitize passes. Loaded on first :Project* invocation, never at startup.
...
local REGISTRY = {
	...
	{ path = ".clang-tidy", category = "lang", when = needs("c"), content = templates.clang_tidy },
	{ path = ".luarc.json", category = "lang", when = needs("lua"), content = templates.luarc },
	{
		-- Only when there is none: a project's own build.zig is the thing you would
		-- least want overwritten.
		path = "build.zig",
		category = "build",
		when = function(d)
			return d.ecosystems.zig == true and vim.uv.fs_stat(vim.fs.joinpath(d.root, "build.zig")) == nil
		end,
		content = function(d)
			return (templates.build_zig:gsub("PROJECT", vim.fn.fnamemodify(d.root, ":t")))
		end,
	},
	...
	{ path = ".prettierrc.json", category = "lang", when = needs("node"), content = templates.prettierrc },
	{
		...
		category = "lang",
		when = function(d)
			return d.ecosystems.python == true and vim.uv.fs_stat(vim.fs.joinpath(d.root, "pyproject.toml")) == nil
		end,
...
function M.parse_args(fargs, bang)
...
function M.doctor()
	...
	if
		vim.uv.fs_stat(vim.fs.joinpath(root, "flake.nix")) ~= nil
		and vim.uv.fs_stat(vim.fs.joinpath(root, "flake.lock")) == nil
	then
...
function M.sanitize()
	...
	vim.ui.select({ "Apply", "Cancel" }, { prompt = table.concat(preview, "\n") }, function(choice)
		...
		for _, rel in ipairs(chmod_paths) do
			vim.uv.fs_chmod(vim.fs.joinpath(root, rel), 493)
		end
		if #junk_paths > 0 then
